...
class TttMenu:
    ...
    def callback_message(self, callback):
      ...
      if(callback.data.isdigit()):
        call_size = callback.data
      else:
        call_move = callback.data[-3] + " " + callback.data[-1]
      board_size = int(call_size)
    ...
    def create_board_markup(self, size):
      ...
      for i in range(size):
        ...
        for j in range(size):
          ...
          row.append(types.InlineKeyboardButton(cell_text, callback_data=f'cell_{i + 1}_{j + 1}'))
        markup.row(*row)
      return markup
